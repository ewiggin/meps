...
import {
    FreshContext,
    Handlers,
} from 'https://deno.land/x/fresh@1.7.2/src/server/types.ts';
import {
...
export const handler: Handlers<unknown> = {
    async GET(_: Request, ctx: FreshContext) {
        ...
        // Assignments
        const assignments =
            (await TerritoryAssignmentService.listByUserId(userId) || [])
                .toSorted((one, other) =>
                    ...
                    {assignments.map((assignment) => (
                        <>
                            <li className='mb-10 ms-4'>
                                    ...
                                    <>
                                        ...
                                        <time className='mb-1 text-sm font-normal leading-none text-gray-400'>
                                            {Intl.DateTimeFormat('es-ES', {
                                                ...
                                                new Date(assignment.closeAt),
                                            )}
                                        </time>
                                        <h3 className='text-lg mb-4 font-semibold text-gray-900'>
                                            📥 Devuelto: {assignment.region}
                                            {' '}
                                ...
                                <h3 className='text-lg mb-4 font-semibold text-gray-900'>
                                    📤 Asignado: {assignment.region}{' '}
                                    {assignment.territoryId}
                                </h3>
                                <a
                                    href={`/admin/maps/${assignment.region}/${assignment.territoryId}`}
                                    target='_blank'
