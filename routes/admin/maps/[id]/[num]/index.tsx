...
export const handler: Handlers<unknown> = {
  async GET(_, ctx) {
    ...
    // Assignments
    const assignments: IKVTerritoryAssignment[] = await TerritoryAssignmentService.list(id, num);
  ...
  async POST(req: Request, ctx) {
    ...
    if (Boolean(formData.get("closeDate"))) {
...
interface IAdminMapsDetailsProps {
  data: {
    ...
    assignments: IKVTerritoryAssignment[];
  };
}
...
export default function AdminMapsDetailFinalPage(
  ...
  // 
  const finalAssignments = assignments.map((item) => item.value)
    .sort((one, other) => new Date(other.date).getTime() - new Date(one.date).getTime());
  ...
  // is Open
  const isOpen = assignments.every((item) => item.value.closeAt);
      ...
      >
        <div class="relative border px-10 py-10">
          <div class="absolute top-0 right-0 w-20">
            <img src={`data:image/svg+xml;charset=utf-8,${svg}`} alt="QR" />
          </div>
        </div>
      </Breadcrumb>
            ...
            <tbody className="divide-y divide-gray-200">
              {finalAssignments
                ...
                <tr>
                  <td className="whitespace-nowrap px-4 py-2 font-medium text-gray-900">
                    <span>
                      {publishers.map((pub) => {
                        return {
                          id: pub.id,
                          name: `${pub.name} ${pub.lastName}`,
                        };
                      }).find(({ id }) => item.userId === id)?.name}
                    </span>
                  </td>
                  <td className="whitespace-nowrap px-4 py-2 text-gray-700">
                    ...
                    {item?.closeAt &&
                      <td className="whitespace-nowrap px-4 py-2 text-gray-700">
                        {new Date(item.closeAt).toLocaleString("es-ES", {
                          year: "numeric",
                          month: "2-digit",
                          day: "2-digit",
                        })}
                  ...
                  <td className="whitespace-nowrap px-4 py-2 text-gray-700 ">
                    {!isOpen && !item?.closeAt && (
                      <form action={`/admin/maps/${id}/${num}`} method="post" class="flex gap-4">
                        <input
                          type="hidden"
                          name="publisher"
                          value={item.userId}
