export class CryptoUtils {
    ...
    static async decrypt(
        receivedJson: { iv: string; data: string },
        keyBase64: string,
    ) {
        ...
        const keyBytes = new Uint8Array(
            [...atob(keyBase64)].map((c) => c.charCodeAt(0)),
        );
        const iv2 = new Uint8Array(
            [...atob(receivedJson.iv)].map((c) => c.charCodeAt(0)),
        );
        const encryptedBytes = new Uint8Array(
            [...atob(receivedJson.data)].map((c) => c.charCodeAt(0)),
        );
